...
{
	if (!F.getSign())
		std::cout << name << " can't execute " << F.getName() << " because: not signed" << std::endl;
	else
	{
		try
		{
			F.execute(*this);
			std::cout << name << " executs " << F.getName() << std::endl;
		}
		catch(const std::exception& e)
		{
			std::cout << name << " can't executs " << F.getName() << " because: "
			<< e.what() << std::endl;
		}
	}
}
